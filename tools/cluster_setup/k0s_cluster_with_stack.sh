...
preflight_checks() {
  ...
  # Worker role classification is positional: the first CPU_WORKER_COUNT worker
  # IPs are treated as CPU nodes, the rest as GPU. That inference drives disk
  # thresholds, node labels, AND which hosts get NVIDIA drivers — so if the
  # configured counts don't sum to the actual worker-IP count, nodes get
  # silently mislabeled and pods sit Pending. Validate the arithmetic up front.
  local _pf_worker_ips=()
  ...
  if (( _pf_cpu + _pf_gpu == _pf_worker_n )); then
    ...
    pf_fail "Worker count mismatch: nodes.cpuWorkers (${_pf_cpu}) + nodes.gpuWorkers (${_pf_gpu}) = $((_pf_cpu + _pf_gpu)), but ${_pf_worker_n} worker IP(s) are listed in nodes.existingIPs.workers. The first ${_pf_cpu} worker IP(s) are labeled CPU and the rest GPU — fix the counts or the IP list so they agree, or GPU drivers/labels will land on the wrong nodes."
  fi
...
PYSCRIPT"
  ...
  local actual_nodes
  actual_nodes=$(ssh_exec "${controller_ip}" "sudo k0s kubectl get nodes --no-headers 2>/dev/null | wc -l" 2>/dev/null || echo "0")
  # Scrub to a bare integer — SSH/wc can emit surrounding whitespace or a
  # multi-line payload, which would make the `-lt` comparison below abort with
  # "integer expression expected" under `set -euo pipefail`. Mirror the
  # join-wait loop above, which already does this.
  actual_nodes=$(echo "${actual_nodes}" | tr -d '[:space:]')
  [[ "${actual_nodes}" =~ ^[0-9]+$ ]] || actual_nodes=0
...
_install_nvidia_on_node() {
  ...
  if [[ -n "${driver_ver}" ]]; then
    ...
    if ! ssh_exec "${gpu_ip}" "
      ...
      # --- Arch guard ---
      # The CUDA repo URLs built below are hardcoded to .../x86_64/... (and the
      # Debian path to ubuntu2204 x86_64). On an arm64 GPU host (Grace-Hopper /
      # GH200) those URLs fetch the wrong repo and fail with a confusing dnf/apt
      # error much later. Fail fast with an actionable message instead.
      ARCH=\$(uname -m)
      if [ \"\${ARCH}\" != 'x86_64' ] && [ \"\${ARCH}\" != 'amd64' ]; then
        echo \"ERROR: NVIDIA driver install in this script only supports x86_64 (got: \${ARCH}).\" >&2
        echo '       For arm64 GPU nodes, pre-install the NVIDIA driver + container toolkit out-of-band and re-run.' >&2
        exit 1
      fi
