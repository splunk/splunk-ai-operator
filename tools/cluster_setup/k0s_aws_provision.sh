...
cmd_provision() {
  ...
  echo ""
  echo "    ssh -i ${KEY_LOCAL} ec2-user@${EIP} 'tail -f ~/install.log'"
  echo ""
  echo "  Step 3 — Destroy when done:"
  echo ""
  echo "    AWS_PROFILE=${AWS_PROFILE:-splunkcloud-ai-dev} \\"
  echo "      $(realpath "$0") destroy --config $(realpath "${CONFIG_FILE}") --yes"
  echo ""
  echo "================================================================"
