...
#### Compaction reclaims rows, not disk space

Every write to a Kubernetes object — including a status-only update that changes
nothing but a timestamp — is an MVCC insert of a brand-new full-object row, never
an in-place update. This is structural (etcd/kine both work this way, not a kine
quirk): old revisions must stay immutable for watch-replay and optimistic-concurrency
conflict detection to work. Compaction deletes revisions once they're superseded and
older than its watermark, which keeps kine's logical row count bounded — but SQLite's
`DELETE` returns freed pages to an **internal freelist for reuse within the file**,
not to the OS. The `.db` file's size on disk is a **high-water mark**: it only shrinks
via an explicit `VACUUM` (a blocking, full-file rewrite — never run automatically here;
see `_kine_disable_startup_vacuum=true` below).

Practically, this means:

- **Steady-state churn is fine.** Node leases, event TTL turnover, the leader-election
  lease, and genuine status changes all get compacted and their pages recycled from
  the freelist. The file stays flat because writes reuse freed pages faster than the
  file needs to grow.
- **A stuck reconcile loop is not fine, even with compaction running.** If a controller
  bug causes an object to be rewritten every few seconds (e.g. an unconditional
  `Status().Update()` with no diff-gate, or a parent controller re-defaulting a
  zero-value spec field every pass so `CreateOrUpdate` sees a spurious diff every
  time), new page allocation can outpace the 5-minute compaction cycle and the file
  grows past its previous high-water mark — permanently, since that growth is never
  reclaimed short of a manual `VACUUM`. This is exactly what happened on
  `ai-cluster-pratyush`: a KubeRay `RayService` object being rewritten every ~2-4s
  (root cause: an app stuck in `DEPLOY_FAILED`, which held KubeRay's reconciler on an
  unconditional-status-write branch) drove the file to 69GB before compaction/rate
  measurement caught it.
- **A lagging watcher can hold back compaction's watermark**, letting rows pile up
  even while compaction is nominally "running" — worth checking for during an
  investigation, not just assuming compaction is keeping up because the process is
  enabled.

**Diagnosing "is churn outpacing reclaim right now"** — file size alone answers "how
big is the historical scar tissue," which is dominated by past bursts, not current
health. A better live signal is the SQLite freelist fraction:

```python
import sqlite3
c = sqlite3.connect("file:/var/lib/k0s/db/state.db?mode=ro", uri=True)
cur = c.cursor()
page_size = cur.execute("PRAGMA page_size").fetchone()[0]
page_count = cur.execute("PRAGMA page_count").fetchone()[0]
freelist_count = cur.execute("PRAGMA freelist_count").fetchone()[0]
print(f"total: {page_size*page_count/1e9:.2f} GB, "
      f"free/reusable: {page_size*freelist_count/1e9:.2f} GB, "
      f"live: {page_size*(page_count-freelist_count)/1e9:.2f} GB")
```

If `freelist_count` is a large fraction of `page_count`, the file is mostly reusable
space wrapped around a small amount of live data — new writes will consume freelist
pages before the file grows further, and the on-disk size is not itself a problem yet.
A `freelist_count` fraction that's shrinking over time (churn outpacing compaction) is
the actual early-warning signal, not the raw file size or disk-usage percentage.

#### Production recommendations

- **Keep the tuning above plus these kine `dataSource` DSN flags** on every
  production controller: `_busy_timeout=30000`, `_txlock=immediate`,
  `_kine_disable_startup_vacuum=true`, alongside `compact-interval: 5m`.
  Startup `VACUUM` is especially dangerous in production because it blocks the
  entire API server on a full-file rewrite during a restart — the worst possible
  time to add latency.
- **Size disk against the worst burst you've ever had, not current usage.** Keep the
  `.db` file under roughly 25–30% of the volume at all times, since the file only
  grows, never shrinks, absent a manual `VACUUM`.
- **Monitor per-object write rate and the freelist fraction, not just disk %.** Disk
  usage is a lagging indicator — by the time it moves, the damage is already
  permanent. A churn bug shows up as a write-rate anomaly on a specific object key
  (visible by sampling `SELECT COUNT(*), MAX(id) FROM kine WHERE name = ?` over a
  time window) long before it becomes a disk-pressure incident.
- **Treat `VACUUM`/defrag as planned maintenance, not emergency response** — an
  explicit, capacity-gated, off-hours window (stop `k0scontroller` → offline
  `VACUUM` → restart), never automatic and never under pressure, since it blocks the
  API for the duration.
- **Audit every `Status().Update()` / `CreateOrUpdate` call for a real diff-gate**
  before merging new controllers or features. This bug class (unconditional status
  writes, and spec builders that don't match persisted defaults) is the actual lever
  on the write floor — compaction and freelist reuse buy margin, they don't fix a
  reconciler that writes every pass regardless of whether anything changed.
- **If the deployment requires the control plane to survive losing a controller
  node, kine/SQLite cannot provide that regardless of tuning** — it is a
  single-writer shim. That requires a fresh multi-controller cluster on the `etcd`
  storage backend (see [High Availability Setup](#high-availability-setup)); there
  is no supported in-place kine→etcd conversion.
