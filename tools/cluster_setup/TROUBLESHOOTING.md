...
```bash
ls -lt tools/cluster_setup/logs/k0s-install-*.log | head -5
# Open the most recent one and search for ERROR:
grep "ERROR" tools/cluster_setup/logs/k0s-install-*.log | tail -20
```
...
tail -f tools/cluster_setup/logs/k0s-install-$(date '+%Y-%m-%d')*.log

# Search for errors across all logs
grep -h "ERROR" tools/cluster_setup/logs/k0s-install-*.log | sort | uniq -c | sort -rn
```
...
```bash
CONFIG_FILE=./my-cluster.yaml ./k0s_cluster_with_stack.sh diagnose
ls -lh tools/cluster_setup/logs/splunk-ai-diagnose-*.tar.gz | tail -1
```
