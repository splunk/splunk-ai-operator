...
render_feature_env_block_from_config() {
  ...
  feature_env_yaml="$(yq eval '.aiPlatform.features[0].env // {}' "${CONFIG_FILE}" 2>/dev/null || true)"
  if [[ -z "${feature_env_yaml}" || "${feature_env_yaml}" == "null" || "${feature_env_yaml}" == "{}" ]]; then
    return 0
  fi
