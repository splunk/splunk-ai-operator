...
- `hecEndpoint` is generated by the installer for the bundled Splunk Standalone
  and sends platform telemetry to its Splunk HTTP Event Collector on port 8088

An external Splunk integration does not configure `hecEndpoint` through this
workflow. It requires the appropriate JWT issuer under `splunk.trustedIssuers`
and network access to the enabled SAIA and SLIM Routes.
...
instance cannot use the documented SLIM endpoint. The bundled Splunk instance
can still use the internal SLIM service shown above.
...
For an external Splunk instance, this procedure requires
